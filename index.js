...
function indekseGoreFenomen(fenomenler, index) {
  let newArr = fenomenler[index].profile;
  return `${index}. indekste bulunan fenomen: ${newArr}`;
}
...
function profilListesi(fenomenler) {
  let fenomen = fenomenler;
  let profil = [];
  
  for (let i = 0; i < fenomen.length; i++) {
    profil.push(fenomen[i].profile);
...
function enFenomenler(fenomenler) {
  ...
  let newArr1 = fenomenler.slice();
  let takibiBolOlanlar = [];
  for (let i = 0; i < newArr1.length; i++) {
    if (newArr1[i].followers > 100000000) {
      takibiBolOlanlar.push(newArr1[i].profile);
    }
  }
  return takibiBolOlanlar;
...
function fenomenGonderimSayisi(fenomenler, profile){
  let copy2 = fenomenler.slice();
 
  for (let i = 0; i < copy2.length; i++) {
    if (copy2[i].profile == profile) {
      return copy2[i].posts;
    }   
  }  
}
